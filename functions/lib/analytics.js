...
exports.generateAnalyticsReport = exports.trackUserActivity = void 0;
...
async function generateNoticesReport(startDate, endDate) {
    const snapshot = await admin.firestore()
        .collection("notices")
...
async function generateMessagesReport(startDate, endDate) {
    const snapshot = await admin.firestore()
        .collection("messages")
        .where("createdAt", ">=", startDate)
        .where("createdAt", "<=", endDate)
...
function getNoticesByType(notices) {
    const typeCounts = {};
...
function getNoticesByAuthor(notices) {
    const authorCounts = {};
    notices.forEach(notice => {
...
function getMessagesByType(messages) {
    const typeCounts = {};
