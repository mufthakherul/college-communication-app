...
- Direct device-to-device messaging via Bluetooth or WiFi Direct
...
- Maintain connections with active devices
...
1. Discover nearby devices running the same app
2. Attempt to connect to discovered devices
3. Display connected devices in the "Connected Devices" list
...
- **Connected Devices**: Number of active peer connections
...
- **Connected Device List**: Details of each connected peer
...
1. **Bluetooth**: 
   ...
   - Best for close-proximity communication
...
3. **Privacy**:
   - Only share data with explicitly connected peers
...
1. **Keep Bluetooth/WiFi Enabled**:
   - Both need to be on for mesh networking
...
3. **Network Hygiene**:
   ...
   - Regularly refresh the device list
...
**A:** No, mesh networking works completely offline using Bluetooth and WiFi Direct.
...
### Q: Is it secure?
**A:** Yes, only authenticated app users can join, and messages include sender verification.
...
**A:** Currently optimized for text messages and notices. Large files may be slow over mesh network.
