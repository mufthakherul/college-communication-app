...
class _FormulaSheetScreenState extends State<FormulaSheetScreen> {
  ...
  final Map<String, List<Formula>> _formulas = {
    'Mathematics': [
      Formula('Quadratic Formula', 'x = (-b ± √(b² - 4ac)) / 2a', 'For equation ax² + bx + c = 0'),
      Formula('Pythagorean Theorem', 'a² + b² = c²', 'For right-angled triangles'),
      ...
      Formula('Distance Formula', 'd = √[(x₂-x₁)² + (y₂-y₁)²]', 'Distance between two points'),
      Formula('Slope of Line', 'm = (y₂-y₁)/(x₂-x₁)', 'Slope between two points'),
      Formula('Sum of n terms', 'Sₙ = n(a + l)/2', 'For arithmetic progression'),
      Formula('Compound Interest', 'A = P(1 + r/n)^(nt)', 'P: principal, r: rate, n: times per year, t: years'),
    ],
    'Physics': [
      Formula('Newton\'s Second Law', 'F = ma', 'Force = mass × acceleration'),
      Formula('Kinetic Energy', 'KE = ½mv²', 'm is mass, v is velocity'),
      Formula('Potential Energy', 'PE = mgh', 'm is mass, g is gravity, h is height'),
      ...
      Formula('Gravitational Force', 'F = G(m₁m₂)/r²', 'Newton\'s law of gravitation'),
    ],
    'Chemistry': [
      Formula('Ideal Gas Law', 'PV = nRT', 'Pressure × Volume = moles × gas constant × Temperature'),
      ...
      Formula('Dilution', 'M₁V₁ = M₂V₂', 'Concentration and volume relationship'),
      Formula('Percent Composition', '% = (mass of element / total mass) × 100', 'Percentage by mass'),
      Formula('Avogadro\'s Number', 'N = 6.022 × 10²³', 'Number of particles in a mole'),
      Formula('Heat Capacity', 'Q = mcΔT', 'Heat = mass × specific heat × temperature change'),
      Formula('Kinetic Molecular Theory', 'KE_avg = (3/2)kT', 'Average kinetic energy of gas molecules'),
    ],
    'Electronics': [
      Formula('Ohm\'s Law', 'V = IR', 'Voltage = Current × Resistance'),
      Formula('Power', 'P = VI = I²R = V²/R', 'Electrical power formulas'),
      Formula('Series Resistance', 'R_total = R₁ + R₂ + R₃...', 'Sum of all resistances'),
      Formula('Parallel Resistance', '1/R_total = 1/R₁ + 1/R₂ + 1/R₃...', 'Reciprocal sum'),
      ...
      Formula('Capacitive Reactance', 'X_C = 1/(2πfC)', 'Reactance of capacitor'),
      Formula('Impedance', 'Z = √(R² + (X_L - X_C)²)', 'Total opposition to AC'),
